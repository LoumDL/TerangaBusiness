  ...
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="w-full max-w-sm">
      ...
      <div class="text-center mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Payer avec Orange Money</h1>
        <p class="text-sm text-gray-500 mt-1">Entrez votre numéro Orange pour recevoir la demande</p>
      </div>

      <!-- Montant -->
      <div class="bg-orange-50 border border-orange-200 rounded-2xl p-4 mb-6 text-center">
        <p class="text-xs text-orange-600 font-medium mb-1">Montant à payer</p>
        <p class="text-3xl font-bold text-orange-700">{{ formatMontantFCFA(montant) }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div v-if="step === 'phone'">
          ...
          <div class="flex items-center border-2 rounded-xl overflow-hidden transition-all"
            :class="error ? 'border-red-400' : 'border-gray-200 focus-within:border-orange-400'"
          >
          ...
          <p class="mt-1.5 text-xs text-gray-400">Numéros Orange : 77, 78</p>

          <button
            @click="handleConfirm"
            :disabled="loading"
            class="mt-5 w-full py-3.5 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-200 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all flex items-center justify-center space-x-2"
          >
            <span v-if="loading" class="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            <span>{{ loading ? 'Envoi en cours…' : 'Confirmer le paiement' }}</span>
          </button>
        </div>

        <!-- Étape 2 : Attente USSD -->
        <div v-else-if="step === 'waiting'" class="text-center space-y-5">
          ...
          <div>
            <h3 class="font-bold text-gray-900">Demande USSD envoyée !</h3>
            <p class="text-sm text-gray-500 mt-1">
              Composez <span class="font-bold text-orange-600">#144#</span> ou confirmez la notification
              Orange Money sur votre téléphone
              <span class="font-semibold text-gray-700">+221 {{ formatPhone(phone) }}</span>.
            </p>
          </div>
          <div class="w-full bg-gray-100 rounded-full h-1.5">
            <div class="bg-orange-400 h-1.5 rounded-full animate-pulse" style="width: 60%" />
          </div>
          <p class="text-xs text-gray-400">Vérification… ({{ countdown }}s restantes)</p>
          <button @click="step = 'phone'" class="text-xs text-gray-400 hover:text-gray-600 underline">
            Changer de numéro
          </button>
        </div>
        ...
        <div v-else-if="step === 'result'" class="text-center space-y-4">
          <div v-if="finalStatus === 'VALIDÉ'" class="space-y-3">
            ...
            <p class="text-sm text-gray-500">{{ formatMontantFCFA(montant) }} débité depuis Orange Money.</p>
          </div>
          <div v-else class="space-y-3">
            ...
            <button @click="step = 'phone'" class="text-sm text-orange-500 font-medium underline">Réessayer</button>
          </div>
          <NuxtLink to="/dashboard" class="block w-full mt-3 py-3 bg-navy-500 hover:bg-navy-600 text-white font-semibold rounded-xl text-sm transition-all">
...
useHead({ title: 'Payer avec Orange Money' })
...
const handleConfirm = async () => {
  ...
  try {
    const response = await fetch(`${config.public.apiBase}/api/v1/paiements/confirm`, {
      ...
      body: JSON.stringify({ paiement_id: paiementId.value, phone: cleaned, channel: 'orange' }),
    })
    if (!response.ok) {
...
const startPolling = () => {
  ...
  setTimeout(() => {
    clearInterval(pollInterval)
    clearInterval(countdownInterval)
    if (step.value === 'waiting') { finalStatus.value = 'EN_ATTENTE'; step.value = 'result' }
  }, 60000)
}

onUnmounted(() => { clearInterval(pollInterval); clearInterval(countdownInterval) })
</script>
